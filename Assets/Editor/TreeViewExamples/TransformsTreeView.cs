...
namespace UnityEditor.TreeViewExamples
{
	...
	{
        public  Dictionary<TreeViewItem,bool> treeItemDatasDic=new Dictionary<TreeViewItem, bool>();
		...
		protected override IList<TreeViewItem> BuildRows(TreeViewItem root)
		{
			...
			rows.Clear (); treeItemDatasDic.Clear();
			...
			foreach (var gameObject in gameObjectRoots)
			{
                ...
                treeItemDatasDic.Add(item, true);
		...
		void AddChildrenRecursive (GameObject go, TreeViewItem item, IList<TreeViewItem> rows)
		{
			...
			for (int i = 0; i < childCount; ++i)
			{
                ...
                treeItemDatasDic.Add(childItem, true);
            }
		}
		...
		protected override void RowGUI (RowGUIArgs args)
		{
           ...
           TreeViewItem item= args.item;
			...
			bool isCheck = EditorGUI.Toggle(toggleRect, treeItemDatasDic[item]);
            if (EditorGUI.EndChangeCheck())
            {
                treeItemDatasDic[item] = isCheck;
                ChenkChildren(item, isCheck);
            }
            // Text
            base.RowGUI(args);
		}
        //递归查询
        private void ChenkChildren(TreeViewItem item, bool isCheck)
        {
            if (item.hasChildren)
            {
                foreach (var itemChren in item.children)
                {
                    ...
                    treeItemDatasDic[itemChren] = isCheck;
                    ChenkChildren(itemChren, isCheck);
                }
            }
        }
